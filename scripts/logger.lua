...
local OUTPUT_FOLDER = Mod.NAME .. "/logs"
...
function Logging.log(message)
  if (Mod.runtime) then
    if not (output_file) then
      output_file = OUTPUT_FOLDER .. "/" .. Mod.runtime.game_id() .. ".log"
    end
    local time = ticks_to_timestring()
    game.print({"log.console-format", time, Mod.NAME, Mod.runtime.game_id(), message})
    local output = "[" .. time .. "] (" .. Mod.runtime.game_id() .. ") " .. message .. "\n"
    game.write_file(output_file, output, true)
  else
    log(message)
  end
end
...
function Logging.debug(message)
  if (Mod.settings.startup().debug_mode) then Logging.log(message) end
end
